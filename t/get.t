...
$h->push_header("Accept-Encoding" => 'gzip');
...
if ($res->is_success) {
    is($res->header("Content-Encoding"), "gzip", "The Content-Encoding should return gzip");
...
# Check the outcome of the response
if ($res->is_success) {
    isnt($res->header("Content-Encoding"), "gzip", "The Content-Encoding should return NULL");
    like($res->header("X-Cache"), qr/MISS.*/, "X-Cache: MISS at first time");
    $res->decoded_content( raise_error => 1 );
}
else {
